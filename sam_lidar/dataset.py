  ...
  def __getitem__(self, idx):
    ...
    returns
        annotated_image (dict) with keys:
            ...
            box: Nx4 ndarray of ground truth bounding boxes in XYXY format
            mask: NxHxW ndarray of ground truth masks
    '''
    ...
    mask_path = os.path.join(self.ann_folder, "Masks", f"{basename}.npy")
    masks = np.load(mask_path)

    annotated_image = {'basename': basename, 'rgb':rgb_img, 'tif':rast_img, 'lidar':las, 'box':boxes, 'mask':masks}
