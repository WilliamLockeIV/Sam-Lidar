...
def per_tree_metrics(truths, preds, detection_threshold=0.5, segmentation_threshold=0.5):
    ...
    # Calculate object-based precision, recall, and f1 for all trees across all images
    ...
    # Average pixel-based precision, recall, f1, and IoU for all trees across all images
...
def per_box_metrics(truths, preds, detection_threshold=0.5):
    '''
    Calculate object-based metrics for bounding boxes. Used for an experiment in the paper comparing
    bounding boxes and masks.

    params:
        truths (list): List of sv.Detections objects. Each Detections object includes the ground truth
                    bounding boxes for a single image.
        preds (list): List of sv.Detections objects. Each Detections object includes the predicted
                    bounding boxes for a single image.
        detection_threshold (float): Minimum IoU between predicted and ground truth boxes to count
                                    as a True Positive for object-based metrics
    returns:
        bounding box precision, recall, and f1 (numpy array)
    '''
    # List of ground truths and list of predictions should be same length (i.e. same number of images with
    # ground truth labels and predicted labels)
    assert len(truths) == len(preds)
    ...
    # Calculate bounding box and precision, recall, and f1 for all trees across all images
    box_tp = sum(box_tp)
    box_fp = sum(box_fp)
    box_fn = sum(box_fn)
    ...
    return box_precision, box_recall, box_f1